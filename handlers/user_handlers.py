...
from keyboards import menu_kb, search_type_kb, download_kb, episodes_kb, payment_confirm_kb, vip_plans_kb, download_options_kb, select_episode_kb
...
FIRE      = ae("5346082673938960600", "🔥")   # olov
...
async def get_web_app_url_value():
...
async def build_episode_caption(anime_id: int, ep_num: int, anime_name: str, bot) -> str:
    ...
    AE = '<tg-emoji emoji-id="5346082673938960600">🔥</tg-emoji>'
...
async def show_anime(message: Message, anime_id: int):
    ...
    web_url = await get_web_app_url_value()
    from keyboards import download_kb as dkb
    kb = dkb(anime_id, likes, dislikes, web_url)
...
async def like_callback(callback: CallbackQuery):
    ...
    from keyboards import download_kb as dkb
    try:
        await callback.message.edit_reply_markup(reply_markup=dkb(anime_id, likes, dislikes, web_url))
    except:
        pass
