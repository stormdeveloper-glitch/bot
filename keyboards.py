...
def download_kb(anime_id: int, likes: int = 0, dislikes: int = 0, web_app_url: str = ""):
    ...
    if web_app_url:
        if not web_app_url.startswith("https://"):
            web_app_url = "https://" + web_app_url.lstrip("http://").lstrip("https://")
        keyboard.append([InlineKeyboardButton(text="🌐 Web App", web_app=WebAppInfo(url=web_app_url))])
    keyboard += [
...
def download_options_kb(anime_id: int):
    """Yuklash tugmasi bosilganda chiqadigan 2ta variant."""
    keyboard = [
...
def select_episode_kb(anime_id: int, episodes: list, page: int = 0):
    """Qismlab yuklash uchun qism tanlash klaviaturasi."""
...
def episodes_kb(anime_id: int, current_ep: int, total_eps_data: list, page: int = 0):
    ...
    for q in page_eps:
        if q == current_ep:
            ...
            buttons.append(InlineKeyboardButton(text=f"🔵 {q}", callback_data=f"yuklanolish={anime_id}={q}"))
